...
import { AlertCircle, PlusCircle, Activity, Heart, Phone, MapPin, CheckCircle, Navigation as NavIcon, Users, MessageSquare, X, ExternalLink } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-950 text-slate-100">
              ...
              {requests.length === 0 ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {requests.map((req) => (
                    <div
                      key={req.id}
                      className={`border p-5 rounded-3xl transition relative overflow-hidden flex flex-col justify-between min-h-[190px] ${req.is_emergency
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <div className={`font-black text-sm px-3.5 py-2 rounded-2xl shadow-sm ${req.is_emergency
                      ...
                      <div className="mt-6 pt-4.5 border-t border-slate-900/60 flex justify-between items-center text-xs gap-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="bg-slate-900/40 hover:bg-slate-800/40 text-slate-300 font-bold px-3 py-1.5 rounded-xl border border-slate-800/50 hover:border-slate-700 transition cursor-pointer"
                          >
                            Details
                          </button>
                          <a
      ...
      {selectedRequest && (
        ...
        >
          <div
            className="relative w-full max-w-lg glass rounded-3xl border border-slate-800/40 p-6 flex flex-col gap-6 shadow-2xl max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-slate-900/60 pb-4">
              <div>
                <div className="flex items-center gap-2.5">
                  <h3 className="font-extrabold text-slate-100 text-xl tracking-tight">
                    {selectedRequest.first_name} {selectedRequest.last_name}
                  </h3>
                  {selectedRequest.is_emergency && (
                    <span className="bg-red-600 text-white font-black text-[10px] px-2 py-0.5 rounded-md uppercase pulse-sos">
                      SOS Emergency
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  Active Request • Posted {new Date(selectedRequest.created_at).toLocaleDateString()}
                </p>
              </div>
              <button
                onClick={() => {
                ...
                className="text-slate-400 hover:text-slate-100 p-1.5 hover:bg-slate-900/50 rounded-xl transition duration-150 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-gradient-to-r from-red-600/10 to-rose-600/10 border border-red-500/20 p-4.5 rounded-2xl flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-red-400">Required Blood Group</p>
                  <p className="text-sm font-semibold text-slate-300 mt-1">
                    Compatible donors are requested to respond
                  </p>
                </div>
                <div className="bg-red-600 text-white font-black text-2xl w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg shadow-red-950/20 shrink-0">
                  {selectedRequest.blood_type}
                </div>
              </div>

              {modalError && (
                <div className="bg-red-950/20 border border-red-900/30 p-4 rounded-2xl text-red-400 text-xs flex items-center gap-3">
              ...
              {/* Location Details */}
              <div className="flex flex-col gap-2.5 bg-slate-950/40 border border-slate-900 p-4.5 rounded-2xl">
                <p className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Location & Directions</p>
                <div className="flex items-start gap-3 mt-1">
                  <MapPin className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-200">{selectedRequest.location}</p>
                    {selectedRequest.latitude && selectedRequest.longitude ? (
                      <>
                        <p className="text-xs text-slate-400 mt-1.5">
                          GPS Coordinates: {selectedRequest.latitude.toFixed(6)}, {selectedRequest.longitude.toFixed(6)}
                        </p>
                        <a
                          href={`https://www.google.com/maps/search/?api=1&query=${selectedRequest.latitude},${selectedRequest.longitude}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="mt-3 inline-flex items-center gap-1.5 bg-slate-900/60 hover:bg-slate-800/70 text-slate-200 text-xs font-bold px-3 py-2 rounded-xl border border-slate-800/65 transition cursor-pointer"
                        >
                          <NavIcon className="w-3.5 h-3.5 text-red-500" />
                          View on Google Maps
                      ...
                      <p className="text-xs text-slate-500 italic mt-1.5">
                        GPS coordinates not logged for this request.
                      </p>
                    )}
                  </div>
                </div>
              ...
              {/* Contact Information */}
              <div className="flex flex-col gap-2.5 bg-slate-950/40 border border-slate-900 p-4.5 rounded-2xl">
                <p className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Contact Details</p>
                <div className="flex items-center gap-3 mt-1">
                  <Phone className="w-5 h-5 text-slate-400 shrink-0" />
                  <p className="text-sm font-bold text-slate-200">{selectedRequest.contact_number}</p>
                </div>
              </div>
            </div>

            {/* Modal Actions */}
            <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-slate-900/60">
              <button
                onClick={() => {
                  const currentUser = getCurrentUser();
                  if (!currentUser) {
                  ...
                  if (selectedRequest.requester_id === currentUser.id) {
                    setModalError('You cannot start a chat with yourself.');
                    return;
                  }
                  if (!selectedRequest.requester_id) {
                    setModalError('This request does not support in-app chat (guest request).');
                    return;
                  }
                  setSelectedRequest(null);
                  setModalError('');
                  router.push(`/chat?other_id=${selectedRequest.requester_id}`);
                }}
                className="flex-1 flex items-center justify-center gap-2 bg-slate-900/60 hover:bg-slate-800/70 text-slate-200 font-bold px-4 py-3 rounded-2xl border border-slate-800/60 transition cursor-pointer"
              >
              ...
              <a
                href={`https://wa.me/${selectedRequest.contact_number.replace(/[^\d]/g, '')}?text=Hello%20${selectedRequest.first_name},%20I%20am%2520responding%2520to%2520your%2520BloodHero%2520blood%2520request.`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 text-white font-bold px-4 py-3 rounded-2xl shadow-lg shadow-red-950/20 transition hover:scale-[1.02] transform duration-150 cursor-pointer"
              >
